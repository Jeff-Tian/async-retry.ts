...
async function handleException(ex, handlers, retry, retryResult, logger = console) {
  ...
  try {
    ...
    for (let i = 0; i < handlers.length; i++) {
      fixed = await tryHandleError(handlers[i].error, ex, handlers[i].handler, retry, retryResult, logger);
    ...
    try {
      ...
      try {
        await handleException(
          ...
          handlers,
          async () => {
            return RetryAsyncAction.retry(action, maxRetryCount - 1, handlers, logger);
          },
          retryResult,
          logger,
        );
